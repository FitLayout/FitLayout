...
public final class VipsOutput {
	 ...
	 * @return Source code
	 */
	private String getSource(Box node)
	{
		String content = "";
		/*try
		{
	...
	private void writeVisualBlocks(Element parentNode, VisualStructure visualStructure)
	{
		...
		layoutNode.setAttribute("DOMCldNum", String.valueOf(parentBox.getChildCount()));
		...
		if (_pDoC >= visualStructure.getDoC())
		{
			...
			if (visualStructure.getChildrenVisualStructures().size() == 0)
			{
				if (visualStructure.getNestedBlocks().size() > 0)
				{
					...
					{
						Box elementBox = block.getBox();

						if (elementBox == null)
							continue;

						if (!"XDiv".equalsIgnoreCase(elementBox.getTagName()) &&
								!"XSpan".equalsIgnoreCase(elementBox.getTagName()))
							src += getSource(elementBox);
						else
							src += elementBox.getOwnText();

						content += elementBox.getOwnText() + " ";
	 ...
	 */
	public void writeXML(VisualStructure visualStructure, Page pageViewport)
	{
		try
		{
			...
			String pageTitle = pageViewport.getTitle();

			vipsElement.setAttribute("Url", pageViewport.getSourceURL().toString());
			...
			vipsElement.setAttribute("order", String.valueOf(pageViewport.getOrder()));*/ //TODO?
